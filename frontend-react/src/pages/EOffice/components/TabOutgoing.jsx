...
import { Search, PlusCircle, RefreshCw, FileText, Download, Edit, Trash2, Calendar, User, Eye, Send } from 'lucide-react';
import Modal from '../../../components/Modal';
import ModernDatepicker from '../../../components/ModernDatepicker';

export default function TabOutgoing() {
    ...
    const fetchMails = async () => {
        try {
            const res = await api.get('/mails?type=outgoing');
            setMails(res.data);
        } catch (err) {
            console.error(err);
        }
    ...
    const openCreate = () => {
        setFormData({ reference_number: '', entity: '', subject: '', date: '', file: null });
        setEditId(null);
        setIsOpen(true);
    };
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-48">
                    ...
                    <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                        placeholder="Cari No. Surat, Tujuan, atau Perihal..." className="input-dark pl-9 text-sm w-full"/>
                </div>
                <button onClick={fetchMails} className="btn-ghost"><RefreshCw size={13}/> Refresh</button>
                <button onClick={openCreate} className="btn-primary"><Send size={13}/> Tambah Surat Keluar</button>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg-table-head)' }}>
                            ...
                            <th className="py-3 px-4 text-xs font-semibold uppercase text-gray-400">Penerima/Tujuan</th>
                            <th className="py-3 px-4 text-xs font-semibold uppercase text-gray-400">Tanggal Kirim</th>
                            <th className="py-3 px-4 text-xs font-semibold uppercase text-gray-400">Arsip File</th>
                            <th className="py-3 px-4 text-xs font-semibold uppercase text-gray-400">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filtered.length === 0 ? (
                            <tr><td colSpan={5} className="py-8 text-center text-sm text-gray-400">Belum ada data surat keluar</td></tr>
                        ) : filtered.map((m, i) => (
                                ...
                                <td className="py-3 px-4">
                                    {m.file_path ? (
                                        <a href={`${baseURL}/storage/${m.file_path}`} target="_blank" rel="noreferrer" 
                                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500/20">
                                            <FileText size={12}/> Unduh Arsip
                                        </a>
            ...
            <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={editId ? 'Edit Surat Keluar' : 'Tambah Surat Keluar'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold mb-1 text-gray-400">Tujuan Instansi/Pribadi *</label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold mb-1 text-gray-400">Upload Softcopy Surat (Opsional)</label>
